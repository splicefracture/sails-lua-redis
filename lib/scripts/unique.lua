...
local _meta      = KEYS[2]
local _attribute = KEYS[3]
local _criteria  = KEYS[4]
...
for _,idx in pairs(index) do
    if values[idx] and type(values[idx]) == "number" then
        local iresp, ierr = redis.call("zrem", "waterline:"..key..":"..idx, primary); 
        local iresp, ierr = redis.call("zadd", "waterline:"..key..":"..idx, values[idx], primary); 
    end
end

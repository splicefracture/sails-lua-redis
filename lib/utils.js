...
utils.filterRange = function(key){
    
    if(key == ">=" || key == "greaterThanOrEqual" || key == ">" || key == "greaterThan"){
        return "min";
    }
    if(key == "<=" || key == "lessThanOrEqual" || key == "<" || key == "lessThan"){
        return "max";
    }
    return key;
}
...
function matchObject(test_value,record_value){
...
utils.match = function match(record, criteria) {
    ...
    return _.reduce(where,function(memo,item){
        ...
        if (_.isArray(test_value) ){
            ...
            if (_.every(test_value, _.isObject)){
                return _.reduce(test_value,function(memo,value){
                    return memo && matchObject(value,record_value);
                }, true)
            }else{
                return ~_.indexOf(test_value,record_value);    
            }
        }else if(_.isObject(test_value)){
            return matchObject(test_value,record_value);
        }else{
